...
import { PRIMARY_ROUTE, INITIAL_SHIPMENT } from '../constants';
...
interface DashboardProps {
    ...
    currentDisruptionEvent?: DisruptionEvent;
    lastUpdateTime?: Date;
}
...
const Dashboard: React.FC<DashboardProps> = ({
    ...
    return (
        <div className="bg-gray-100/80 dark:bg-gray-900/80 backdrop-blur-sm p-4 md:p-6 flex flex-col space-y-4 custom-scrollbar h-full">
            ...
            <div className="bg-white dark:bg-gray-800 p-4 rounded-lg border border-gray-200 dark:border-gray-700">
                <h2 className="text-lg font-semibold text-cyan-500 dark:text-cyan-400 mb-3">Route Map</h2>
                <RouteMap 
                    routes={allRoutes} 
                ...
                <div className="mt-3 flex flex-wrap gap-4 text-xs">
                    ...
                    <div className="flex items-center">
                        <div className="w-4 h-1 bg-gray-500 mr-2 opacity-40"></div>
            ...
            <div className="flex-grow bg-white dark:bg-gray-800 p-4 rounded-lg border border-gray-200 dark:border-gray-700 flex flex-col">
                ...
                <div className="space-y-4">
                    {allRoutes.map(route => {
                        const routeRisk = riskProfile[route.id] || { overall: 0, factors: { congestion: 0, tariffs: 0, unrest: 0 } };
                        const riskPercentage = (routeRisk.overall * 100).toFixed(0);
                        const isActive = activeRoute.id === route.id;
                        
                        return (
                            <div key={route.id} className={`p-4 rounded-lg border-2 ${isActive ? 'bg-cyan-50 dark:bg-cyan-900/50 border-cyan-400' : 'bg-gray-200/50 dark:bg-gray-700/50 border-gray-300 dark:border-gray-600'}`}>
                                <div className="flex items-center justify-between mb-2">
                                    <h3 className="font-bold text-sm">{route.name} {isActive && <span className="text-xs text-cyan-500 dark:text-cyan-400 ml-2">(ACTIVE)</span>}</h3>
                                    <span className={`text-sm font-bold px-2 py-1 rounded ${
                                        routeRisk.overall > 0.6 ? 'bg-red-100 dark:bg-red-900/50 text-red-700 dark:text-red-300' :
                                        routeRisk.overall > 0.3 ? 'bg-yellow-100 dark:bg-yellow-900/50 text-yellow-700 dark:text-yellow-300' :
                                ...
                                <div className="mb-2">
                                    <RiskBar value={routeRisk.overall} />
                                </div>
                                <div className="grid grid-cols-3 gap-2 text-xs mt-2">
                                    <div>
                                        <span className="text-gray-500 dark:text-gray-400">Congestion:</span>
                                        <span className="ml-1 font-semibold">{(routeRisk.factors.congestion * 100).toFixed(0)}%</span>
                                    </div>
                                    <div>
                                        <span className="text-gray-500 dark:text-gray-400">Tariffs:</span>
                                        <span className="ml-1 font-semibold">{(routeRisk.factors.tariffs * 100).toFixed(0)}%</span>
                                    </div>
                                    <div>
                                        <span className="text-gray-500 dark:text-gray-400">Unrest:</span>
                                        <span className="ml-1 font-semibold">{(routeRisk.factors.unrest * 100).toFixed(0)}%</span>
                                    </div>
                                </div>
                            </div>
                        );
                ...
                {isDisruptionTriggered && currentDisruptionEvent && (
                    <div className="mt-6 p-4 bg-gray-200/50 dark:bg-gray-700/50 rounded-lg border border-gray-300 dark:border-gray-600 animate-fade-in-up">
                        ...
                        <div className="border-l-2 border-yellow-500 dark:border-yellow-400 pl-3">
                            <p className="font-semibold">{currentDisruptionEvent.name}</p>
                            <p className="text-sm text-gray-600 dark:text-gray-300">{currentDisruptionEvent.description}</p>
                            {lastUpdateTime && (
                                <p className="text-xs text-gray-500 dark:text-gray-400 mt-2 italic">
                                    Last updated: {lastUpdateTime.toLocaleTimeString()} (Updates every 10 seconds)
                                </p>
                            )}
                            <div className="mt-4">
                                ...
                                {isAiLoading ? (
                                    <div className="mt-2 flex items-center space-x-2">
                                        ...
                                        <p className="text-sm text-gray-500 dark:text-gray-400 italic">Arkham AI agent analyzing routes...</p>
                                    </div>
                                ) : (
                                    <p className="text-sm text-gray-600 dark:text-gray-300 mt-1">{aiAnalysis || 'No recommendation available yet.'}</p>
                                )}
                            </div>
                            {!isAiLoading && !isLoading && activeRoute.id !== PRIMARY_ROUTE.id && (
